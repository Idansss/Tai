...
## 2026-07-17 — TMS-B5-001 the payment flow is available (mock gateway)

- Status: implemented on `codex/b5-payment-provider`, stacked on TMS-B4-003; consume after both PRs merge. Completes the payment half of request TMS-FBR-004.
- **This lets `lib/payment.ts` move off the pure mock resolver.** The provider is a **mock** (no real money) until TMS-B5-002 wires Flutterwave behind the same interface, but the flow, states, and endpoints are the real ones.
- Endpoints (all keyed by the **order reference** so a guest can complete checkout — no session required):
  - `POST /api/v1/orders/{reference}/payment` — start (or resume) payment. Returns the handoff `{ status, provider, reference, redirectUrl }`. Idempotent: reloading does not create a second attempt. Moves the order into `PAYMENT_PROCESSING`.
  - `GET /api/v1/orders/{reference}/payment` — read the current handoff (poll this for status).
  - `POST /api/v1/orders/{reference}/payment/verify` — reconcile against the provider if the webhook seems lost; safe to call repeatedly.
- **Flow:** place the order (TMS-B4-003) → `POST .../payment` → send the browser to `redirectUrl` (the mock hosts a page at `/checkout/mock/{reference}`) → the provider calls our webhook → poll `GET .../payment` (or call `verify`) until `status` is `SUCCEEDED`/`FAILED`. **Never trust a client `?outcome=` param** — the status is only ever set by a signed webhook or a server-side verify.
- **`redirectUrl` is where the customer pays.** In the mock it is a local URL; with a real gateway it is the provider's hosted checkout. Treat it as opaque and just navigate to it.
- **Status is the shared `PaymentStatus` enum;** map it to customer copy, never render the raw code. The order's own `status`/`timeline` (from `GET /orders/{reference}`) still drive the fulfilment view; the payment handoff drives the pay/processing/success/failure screens.
