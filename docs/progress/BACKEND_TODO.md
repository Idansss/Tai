...
- [x] TMS-B5-002 Implement Flutterwave adapter architecture and sandbox handoff
  - Status: Verified (live credential verification Blocked — no keys supplied)
  - Owner: Codex
  ...
  - Implementation evidence: `apps/api/src/payments/flutterwave-payment-provider.ts` implements the `PaymentProvider` port with an injectable `fetch` so it is fully testable without a network. It authenticates webhooks by Flutterwave's `verif-hash` scheme (constant-time compare against the configured hash), generates a `tx_ref` as the provider reference, resolves the numeric transaction id via `verify_by_reference` for verification and refunds, and is the single place that converts between the platform's integer minor units (kobo) and Flutterwave's major units (naira). The module factory selects the provider by `PAYMENT_PROVIDER`; configuration adds `FLUTTERWAVE_BASE_URL`/`FLUTTERWAVE_SECRET_KEY`/`FLUTTERWAVE_WEBHOOK_HASH`, requiring the credentials whenever Flutterwave is selected. `PaymentService`, the webhook endpoint, and the order state machine are unchanged — the adapter drops in behind the same port (ADR-019).
  - Tests: Seven contract tests with a recording fake `fetch` prove the create request shape and naira conversion (1,655,000 kobo → 16,550), `verify_by_reference` mapping back to minor units, exact `verif-hash` acceptance/rejection, webhook parsing and transaction-id dedupe with amount normalisation, transaction-id resolution then full/partial refund in major units, error-envelope handling, and unconfigured health. Configuration tests prove Flutterwave credentials are required when it is selected. Payment suite 3 files / 20 tests. `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm build`, and `pnpm db:validate` pass. No migration.
  - Notes: **Live verification is Blocked** — no Flutterwave sandbox keys are present, so `createPayment`/`verify`/`refund` are proven against a fake `fetch` rather than the live API. The full contract, config validation, webhook handling, and refund flow are complete; supplying `FLUTTERWAVE_SECRET_KEY`/`FLUTTERWAVE_WEBHOOK_HASH` and running one sandbox transaction is all that remains. Flutterwave's `verif-hash` is a static shared secret (not a body HMAC), which is why the adapter compares the header to the configured hash. Amount conversion is the one correctness hazard and is unit-tested in both directions. Stacked on `codex/b5-payment-provider` (TMS-B5-001).
